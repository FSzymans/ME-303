...
{  
	...
	num=num-' ';
	LCD_SetWindows(x,y,x+(zX*size)/2-1,y+(zY*size)-1);
	if(!mode){
		for(pos=0;pos<(size);pos++)
		{
			if(size==12)temp0=asc2_1206[num][pos];
			else temp0=asc2_1608[num][pos];
			temp = temp0;
			for(v=0; v<zY; v++) {
			    for(t=0;t<size/2;t++) {
			        for(u=0; u<zX; u++){
			    ...
			    if(v<(zY-1)) { temp = temp0; }
		    }
			
		}	
	} else { // ne fonctionne pas avec zoom # 1
	        for(pos=0;pos<size;pos++)
	        {
